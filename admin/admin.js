...
async function api(action, body) {
  ...
  if (!response.ok) {
    if (response.status===401 && action!=='login') { $('editor').close(); $('dashboard').hidden=true; $('loginPanel').hidden=false; }
...
function edit(record=null,kind='rental') {
  ...
  $('editorTitle').textContent=record?`Editar registro #${record.id}`:kind==='maintenance'?'Bloquear manutenção':'Nova reserva';$('editor').showModal();
}
...
(async()=>{try{const session=await api('session');csrf=session.csrf;setup=session.setupRequired;if(setup){$('loginTitle').textContent='Crie seu primeiro acesso';$('loginHelp').textContent='No computador local, escolha seu usuário e uma senha de pelo menos 12 caracteres.';$('loginButton').textContent='Criar administrador';$('loginForm').elements.password.minLength=12;$('loginForm').elements.password.autocomplete='new-password';}if(session.authenticated)await load();}catch(error){$('message').textContent='Não foi possível abrir o painel. Acesse pelo servidor PHP (MAMP).';}})();
